...
class Anime {
	...
	related() {
		this.request = '/related';
	}

	screenshots() {
		this.request = '/screenshots';
	}

	franchise() {
		this.request = '/franchise';
	}

	external_links() {
		this.request = '/external_links';
	}
	...
	then(callback, onError = callback) {
		return fetch(`${shikimori.api}/animes/${this.id}${this.request}`)
			.then((r, e) => {
				if (e || r.ok !== true) {
					onError(r, e);
...
class Animes {
	...
	genre(genres) {
		genres.forEach((arg) => {
			if (typeof arg !== 'number') throw 'Genre id must be a number';
		});
	...
	studio(studios) {
		studios.forEach((arg) => {
			if (typeof arg !== 'number') throw 'Studio id must be a number';
		});
	...
	ids(ids) {
		ids.forEach((arg) => {
			if (typeof arg !== 'number') throw 'Id must be a number';
		});
		this.value.ids = ids;
		return this;
	}
	...
	then(callback, onError = callback) {
		...
		const http_query = Object.keys(args)
			.map((k) => {
				if (typeof args[k] === 'object') {
					args[k] = Object.values(args[k])
						.join(',');
				} else {
			...
			.join('&');
		fetch(`${shikimori.apiEndpoint}/animes?${http_query}`)
			.then((r, e) => {
