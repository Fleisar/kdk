(function(){
    ...
    window.kodik.prototype = {
        on(action,handler){
            if(Object.keys(this._a.on).indexOf(action)===-1)throw'This action doesn\'t exist'
            if(typeof handler!=='function')throw'Handler must be a function'
            return this._a.on[action].push(handler),this
        },
        do(action,data){
            if(Object.keys(this._a.do).indexOf(action)===-1)throw'This action doesn\'t exist'
            if(!Object.keys(data).reduce((a,b)=>a&&Object.keys(this._a.do).indexOf(b),true))throw'Invalid construction'
            this.SELF.contentWindow.postMessage({key:`kodik_player_api`,value:{method:action,...data}},'*')
        }
    }
})()
